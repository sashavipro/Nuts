(function($) {
    ...
    $(document).ready(function() {
        ...
        // --- 3. ЛОГИКА ТИПОВ ПОЛЬЗОВАТЕЛЯ (ОБНОВЛЕННАЯ) ---

        function updateFormState() {
            // Смотрим, какой таб активен
            const activeTabType = $('.reg-tab.active').data('type'); // 'physical' или 'legal'
            ...
            if (activeTabType === 'legal') {
                ...
                // Тип: legal
                // Галочка ФОП: скрыта
                // Реквизиты: ПОКАЗАНЫ и ОБЯЗАТЕЛЬНЫ
                ...
                // === ФИЗИЧЕСКОЕ ЛИЦО (ИЛИ ФОП) ===
                $('#fop-block').show(); // Показываем возможность выбрать ФОП

                if (isFopChecked) {
                    // === Выбран ФОП ===
                    // Тип: fop
                    // Реквизиты: СКРЫТЫ (по вашему запросу)
            ...
            // Для отладки
            // console.log("Current User Type:", $userTypeInput.val());
        }
